...
int main(void) {
    ...
    ALLEGRO_SAMPLE_INSTANCE* musica_inst = al_create_sample_instance(musica_fundo);
    ...
    while (running) {
        ...
        if (event.type == ALLEGRO_EVENT_TIMER) {
            ...
            if (estado == JOGO) {
                if (al_key_down(&keyState, ALLEGRO_KEY_RIGHT)) { x += 10; imagem = img_dir; }
                if (al_key_down(&keyState, ALLEGRO_KEY_LEFT)) { x -= 10; imagem = img_esq; }
                if (al_key_down(&keyState, ALLEGRO_KEY_DOWN)) { y += 10; imagem = img_baixo; }
                if (al_key_down(&keyState, ALLEGRO_KEY_UP)) { y -= 10; imagem = img_cima; }
                ...
                if (npc_visivel) {
                    npc_timer += 1.0 / 60.0;
                    if (npc_falando && !npc_som_tocado) {
                        al_play_sample(som_fala_npc, 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
                        npc_som_tocado = true;
                    }
                    if (npc_timer > npc_duracao) npc_visivel = false;
                }
            }
    ...
    al_destroy_sample(som_fala_npc);
    al_destroy_sample(musica_fundo);
    al_destroy_sample_instance(musica_inst);
